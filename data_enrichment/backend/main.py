...
@app.get("/api/ingredient/{product_id}")
async def get_ingredient_detail(product_id: int):
    """Get ingredient detail: suppliers, cross-company usage, inferred flags."""
    conn = get_db_connection()
    ...
    cursor = conn.execute("""
        ...
        WHERE sp.ProductId = ?
    """, (product_id,))
    suppliers = [{"id": r["Id"], "name": r["Name"]} for r in cursor.fetchall()]

    # All BOMs / finished goods that use this ingredient
    cursor = conn.execute("""
        SELECT p.SKU as fg_sku, c.Name as company_name, c.Id as company_id
        FROM BOM_Component bc
        ...
        ORDER BY c.Name
    """, (product_id,))
    used_in = [{"fg_sku": r["fg_sku"], "company": r["company_name"], "company_id": r["company_id"]} for r in cursor.fetchall()]
